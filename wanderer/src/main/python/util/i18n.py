...
def find_resource(dir_name, basename, language_code, ext):
    """
...
def read_text_options(dir_name, basename, language_code, property_name=None, separator='/'):
    ...
    if property_name is None:
        # should be a place text file with one option per line
        path = find_resource(dir_name, basename, language_code, EXT_TEXT)
        contents = read_text_file(path)
        return contents.strip().split('\n')
    else:
        # should be a properties file
        path = find_resource(dir_name, basename, language_code, EXT_PROPERTIES)
        if not path is None:
